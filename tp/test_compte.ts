...
var c1 :Compte; //c1 est une référence
c1 = new Compte(); // création d'une nouvelle instance
//c1._solde = 300; //de l'extérieur de la classe, 
                   //l'accès direct interdit aux éléments privés 
console.log("decouvert autorise:" + Compte.decouvertAutorise);

var tauxInteretDecouvPct = Compte.tauxInteretDecouvertEnPourcentage();
...
try{ c1.solde = -200; //appel interne à set solde(-200)
    console.log("solde de c1="+c1.solde);
}
catch(ex){ console.log("erreur/exception:" + ex);
}
try{ c1.solde = -400; //appel interne à set solde(-400)
    console.log("solde de c1="+c1.solde);
}
catch(ex){ console.log("erreur/exception (c1):" + ex);
}
Compte.decouvertAutorise = -600;
...
try{ c2.solde = -400; //appel interne à set solde(-400)
    console.log("nouveau solde de c2="+c2.solde);
}
catch(ex){ console.log("erreur/exception (c2):" + ex);
}

var c3 = new Compte(3,"compte3",1000.0);
c3.crediter(500);
